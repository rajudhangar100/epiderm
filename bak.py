...
lesion_description_dict = {
    'Actinic keratoses': (
        "Precancerous rough, scaly patches on the skin caused by long-term sun exposure. "
        "They may develop into squamous cell carcinoma if untreated."
    ),
    'Basal cell carcinoma': (
        "A common and slow-growing type of skin cancer that arises from the basal cells. "
        "It rarely spreads but can damage surrounding tissues if ignored."
    ),
    'Benign keratosis-like lesions': (
        "Non-cancerous skin growths that may appear warty or scaly. "
        "They are generally harmless and include seborrheic keratoses and lichen planus-like keratoses."
    ),
    'Dermatofibroma': (
        "A benign skin nodule often caused by minor injuries like insect bites. "
        "It feels firm and is usually harmless, though sometimes itchy or tender."
    ),
    'Melanocytic nevi (normal)': (
        "Commonly known as moles, these are usually benign skin growths made up of pigment-producing cells. "
        "They are generally harmless but should be monitored for changes."
    ),
    'Melanoma': (
        "A serious and aggressive form of skin cancer originating in melanocytes. "
        "Early detection is critical, as melanoma can spread quickly."
    ),
    'Vascular lesions': (
        "Abnormalities of blood vessels including angiomas and hemangiomas. "
        "Usually benign but may need treatment depending on location and size."
    )
}
...
#Show Result:
def show_prediction_popup(result, image_path, description):
    ...
    # Title
    tk.Label(popup, text="🧬 Prediction Summary", font=("Helvetica", 16, "bold"), bg="white", fg="#343a40").pack(pady=(10, 5))

    # Image
    img = Image.open(image_path).resize((320, 200), Image.Resampling.LANCZOS)
    img_photo = ImageTk.PhotoImage(img)
    img_label = tk.Label(popup, image=img_photo, bg="white", bd=2, relief="solid")
    img_label.image = img_photo  # Prevent garbage collection
    ...
    # Predicted Class
    tk.Label(info_frame, text="Predicted Class:", font=("Helvetica", 12, "bold"), bg="white", fg="#007bff").grid(row=0, column=0, sticky="e", padx=5, pady=2)
    tk.Label(info_frame, text=result['predicted_class'], font=("Helvetica", 12), bg="white", fg="#212529").grid(row=0, column=1, sticky="w", padx=5, pady=2)

    # Confidence
    tk.Label(info_frame, text="Confidence Score:", font=("Helvetica", 12, "bold"), bg="white", fg="#007bff").grid(row=1, column=0, sticky="e", padx=5, pady=2)
    tk.Label(info_frame, text=f"{result['confidence']*100:.2f}%", font=("Helvetica", 12), bg="white", fg="#212529").grid(row=1, column=1, sticky="w", padx=5, pady=2)

    # Health Status Alert
    if result['predicted_class'] == "Melanocytic nevi (normal)":
        status_text = "✅ Diagnosis: The patient appears to be healthy. No signs of malignancy detected."
        status_color = "#28a745"  # Green
    else:
        status_text = "⚠️ Alert: Patient requires urgent medical attention and further treatment."
        status_color = "#dc3545"  # Red

    tk.Label(popup, text=status_text, font=("Helvetica", 11, "bold"), bg="white", fg=status_color, wraplength=550, justify="center").pack(pady=(10, 10))

    # Description
    tk.Label(popup, text="About the Disease:", font=("Helvetica", 12, "bold"), bg="white", fg="#28a745").pack(pady=(5, 0))
    tk.Label(popup, text=description, font=("Helvetica", 11), bg="white", fg="#343a40", wraplength=520, justify="left").pack(padx=20, pady=(5, 15))
...
def open_camera():
    ...
    def save_image(pil_img):
        ...
        show_prediction_popup(result,RESULT_IMAGE_PATH,lesion_description_dict[result["predicted_class"]])
